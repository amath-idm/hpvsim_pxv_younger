...
def organize_results(calib, calib_space=None):
    ''' Organize results'''
    result_df = calib.df
    if calib_space is None:
        param_cols = [col for col in result_df.columns.values if col not in ['mismatch', 'index']]
    else:
    ...
    index_list = result_df['index'].values

    for sim_key in calib.sim_results_keys:
        sim_results = [result[sim_key] for result in calib.sim_results]
        if (len(sim_results) != len(index_list)):
            result_df[sim_key] = [sim_results[i] for i in index_list]
        else:
            result_df[sim_key] = sim_results
    ...
    for extra_sim_key in calib.extra_sim_result_keys:
        extra_sim_results = [np.sum(result[extra_sim_key].values) for result in calib.extra_sim_results]
        if (len(extra_sim_results) != len(index_list)):
            result_df[extra_sim_key] = [extra_sim_results[i] for i in index_list]
        else:
            result_df[extra_sim_key] = extra_sim_results
        if 'genotype_dist' in extra_sim_key:
            subcols = calib.glabels
            result_df = expand_array_column(result_df, extra_sim_key, subcols)
        elif isinstance(result_df[extra_sim_key][0], (list, np.ndarray)):
            subcols = [i for i in range(len(result_df[extra_sim_key][0]))]
            result_df = expand_array_column(result_df, extra_sim_key, subcols)

    for analyzer_key in calib.age_results_keys:
        year = [key for key in calib.analyzer_results[0][analyzer_key].keys() if key != 'bins'][0]
        analyzer_results =  [result[analyzer_key][year] for result in calib.analyzer_results]
        if (len(analyzer_results) != len(index_list)):
            result_df[analyzer_key] =[analyzer_results[i] for i in index_list]
        else:
            result_df[analyzer_key] = analyzer_results
        age_bins = calib.analyzer_results[0][analyzer_key]['bins']
        subcols = [f'age_{int(age_bins[i])}-{int(age_bins[i+1]-1)}' if i < len(age_bins)-1 else f'age_{int(age_bins[-1])}+' for i in range(len(age_bins))]
...
def heatmap(param_importance, outcomes, save_plot=False, sort_by=None):
    ...
    plt.figure(figsize=(1*df.shape[1], 0.2*df.shape[0]))
    heatmap = sns.heatmap(np.abs(df), cmap="Blues", linewidths=0.5, linecolor="white")
    heatmap.set_yticks([0.5 + i for i in range(len(df.index))]) 
    # heatmap.set_xticks([0.5 + i for i in range(len(df.columns))])
    heatmap.set_yticklabels(df.index, rotation=0, fontsize=7) 
    heatmap.set_xticklabels(df.columns, fontsize=7)
    plt.show()
    if save_plot:
